...
int main()
{
    ...
    // specify time steps, determine number of observations
    ...
    {
        if (!sensor_obs[t].empty()){observations = sensor_obs[t];}
        else                       {observations = {float(distance_max)};}
        ...
        {
            if (posteriors[p] > 0.001){cout << posteriors[p] << " ";}
            else                      {cout << 0 <<" ";}
        }
        cout << endl;
    }
