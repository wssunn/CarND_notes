...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>> &predictions)
{
   ...
   *   vehicle trajectories as values. 
   *    keys: vehicle id
   *    values: predicted vehicle tragectory 
   *        Trajectories are a vector of Vehicle objects representing
   ...
   * 1. successor_states
   *        Uses the current state to 
   *        return a vector 
   *            of possible successor states for the finite state machine.
   * 2. generate_trajectory
   *        given a state and predictions
   *        Returns a vector of Vehicle objects representing a vehicle trajectory
   *        Note that trajectory vectors might have size 0 if no possible trajectory exists 
   *        for the state. 
   * 3. calculate_cost - Included from cost.cpp, computes the cost for a trajectory.
   */

    // use the current state to return a vector of all possible successor states
    // for all possible successor states:
    //      generate the appropriate trajectory_gen to realize the next state
    //      if trajectory_gen is valid (size not 0):
    //          calculate cost for this trajectory
    //          store cost in vector costs, trajectory in vector final_trajectory
    // find the minimum cost in costs
    // get its index_min
    // return final_trajectory[index_min]

    // use the current state to return a vector of possible successor states
    ...
    for (vector<string>::iterator it = states.begin(); it != states.end(); ++it)
    {
        ...
        //generate_tragectory function: required input of a state and prediction
        //tragectory is a vector of vehicle object
        vector<Vehicle> trajectory = generate_trajectory(*it, predictions);

        //check this is a valid tragectory
        if (trajectory.size() != 0)
        {
            //calculate_cost function: require Vehicle object, map object, vector of Vehicle object
    ...
    // this find the index of tragectory with minimum cost
