...
HBF::maze_path HBF::search(vector< vector<int> > &grid, vector<double> &start, 
                           vector<int> &goal) {
  // Working Implementation of hybrid A* (from breadth search)
  ...
  //create new state object to store information
  maze_s state;
  state.g = g;
  state.x = start[0];
  state.y = start[1];
  ...
  while(!opened.empty()) {
    //sort the states by f-value and start the search with lowest f-value
    //compare_maze_s will compare the f value
